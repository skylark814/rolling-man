var HellLayer = cc.Layer.extend({
    ...
    onMenuCallback() {
        this.sprite.schedule(this.actionUpdate, 1, 10,0.1);
        this.removeChild(this.mn1);
        this.sprite.schedule(this.winOrLose, 0.1, 0, 10.1);
    },
    actionUpdate() {
        ...
        let bz = cc.bezierTo(0.5, bezier);
        let roll = cc.rotateBy(0.5, 360*20);
...
var HellScene = cc.Scene.extend({
    onEnter: function () {
        this._super();
        var layer = new HellLayer();
        this.addChild(layer);
    }
});
